...
他的略覽順序很簡單，跟《原子習慣》一樣沒壓力，看四個地方：','2026-07-09')
...
兩人都很有感的一點是：逛實體書店帶來的好處很難量化。在網路上不管是短影音還是搜尋，內容都被演算法推送；但走進書店、或在鞋櫃旁放一本沒看過的書，多了一個數位時代缺乏的東西——**隨機**。','2026-07-09')
ON CONFLICT(id) DO UPDATE SET content=excluded.content, updated_at=excluded.updated_at;
...
郝哥把買書的痛苦歸納成「四大苦」：買了沒看是一苦、放上書架忘了是一苦、送人但對方未必喜歡又是一苦、越積越多佔空間還是一苦。這四苦的共同根源，都是**沒有連結**。','2026-07-09')
ON CONFLICT(id) DO UPDATE SET content=excluded.content, updated_at=excluded.updated_at;
