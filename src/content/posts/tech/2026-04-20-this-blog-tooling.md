...
D1 是 Cloudflare 的 SQLite-compatible 資料庫，在 Workers 裡直接用 `env.DB.prepare().all()` 查詢，沒有 connection pool、沒有 TCP、沒有跨服務的 IAM 設定。
...
RAG 和 Pagefind 的分工：RAG 回答問題，Pagefind 找精確詞彙。
